...
def calculateMoleculeGroups(natoms, bonds, device):
    ...
    if bonds is not None:
        ...
        nongrouped = torch.tensor([list(group)[0] for group in molgroups if len(group) == 1]).to(device)
        molgroups = [torch.tensor(list(group)).to(device) for group in molgroups if len(group) > 1]
    else:
        molgroups = []
